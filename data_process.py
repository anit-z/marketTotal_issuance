...
def data2df(data):
    ...
    flat_list = [(k, v) for d in data['发行金额'] for k, v in d.items()]

    # Create DataFrame
    df_financial_products = pd.DataFrame(flat_list, columns=['Product Type', 'Issuance Amount'])
    df_financial_products = df_financial_products.sort_values(by='Issuance Amount', ascending=False)
...
def visualization(df):
    ...
    plt.barh(df['Product Type'][::-1], df['Issuance Amount'][::-1])  # Reverse the order for descending bar plot
    plt.xlabel('Value')
    plt.ylabel('Type')
    plt.title('Product release scale')
    plt.grid(True)
    plt.show()
...
if __name__ == '__main__':
    ...
    file_name = 'marketTotal_issuance'
